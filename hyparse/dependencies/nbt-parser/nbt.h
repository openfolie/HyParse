...
static nbt_tag_t *nbt__parse(nbt__read_stream_t *stream, int parse_name,
  ...
  case NBT_TYPE_LIST: {
    ...
    tag->tag_list.value =
        (nbt_tag_t **)NBT_MALLOC(tag->tag_list.size * sizeof(nbt_tag_t *));
    ...
    for (;;) {
      ...
      if (inner_tag->type == NBT_TYPE_END) {
        ...
        tag->tag_compound.value = (nbt_tag_t **)NBT_REALLOC(
            tag->tag_compound.value,
            (tag->tag_compound.size + 1) * sizeof(nbt_tag_t *));
  ...
  case NBT_TYPE_INT_ARRAY: {
    ...
    tag->tag_int_array.value =
        (int32_t *)NBT_MALLOC(tag->tag_int_array.size * sizeof(int32_t));
  ...
  case NBT_TYPE_LONG_ARRAY: {
    ...
    tag->tag_long_array.value =
        (int64_t *)NBT_MALLOC(tag->tag_long_array.size * sizeof(int64_t));
...
nbt_tag_t *nbt_parse(nbt_reader_t reader, int parse_flags) {
  ...
  if (compressed) {
    ...
    int ret = inflateInit2(&stream, gzip_format ? -Z_DEFAULT_WINDOW_BITS
    ...
    do {
      stream.avail_in =
          reader.read(reader.userdata, in_buffer, NBT_BUFFER_SIZE);
...
void nbt__put_byte(nbt__write_stream_t *stream, uint8_t value) {
  if (stream->offset >= stream->alloc_size - 1) {
    stream->buffer =
        (uint8_t *)NBT_REALLOC(stream->buffer, stream->alloc_size * 2);
    stream->alloc_size *= 2;
  }
...
void nbt__write_tag(nbt__write_stream_t *stream, nbt_tag_t *tag, int write_name,
                    int write_type) {
...
void nbt_write(nbt_writer_t writer, nbt_tag_t *tag, int write_flags) {
  ...
  if (compressed) {
    ...
    if (deflateInit2(&stream, NBT_COMPRESSION_LEVEL, Z_DEFLATED, window_bits, 8,
...
void nbt_tag_list_append(nbt_tag_t *list, nbt_tag_t *value) {
  list->tag_list.value = NBT_REALLOC(
...
void nbt_tag_compound_append(nbt_tag_t *compound, nbt_tag_t *value) {
  compound->tag_compound.value =
      NBT_REALLOC(compound->tag_compound.value,
